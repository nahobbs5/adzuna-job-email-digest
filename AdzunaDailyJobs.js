...
//    a) runHourlyJobSearch — Hour timer — Every 2 hours (collects jobs)
...
// Every morning at 8am (Mon-Sat): compiles everything found since yesterday's email,
// sends one digest email, clears the stored results.
// No fits found = no email sent.
// Skips Sundays entirely - both collection & sending.
//
...
// SUNDAY CHECK — returns true if today is Sunday
// ============================================================
function isSunday() {
...
// Filter non-Virginia and non-remote listings
  allListings = allListings.filter(function(job) {
    if (!location) return true; // remote query — keep all
    var jobLocation = (job.location ? job.location.display_name : "").toLowerCase();
    return jobLocation.indexOf("virginia") !== -1;
  });

  Logger.log("Listings after dedup and domain filter: " + allListings.length);
  ...
  // Store fits in PropertiesService for daily digest
  storeFits(fits);
}
...
function storeFits(newFits) {
  ...
  // Deduplicate against already stored jobs by URL
  var storedUrls = {};
...
Evaluate each listing honestly. Include positions where the candidate is a reasonable fit.
Do not overstate qualifications. Active Directory experience is very limited.
